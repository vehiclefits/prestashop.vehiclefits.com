...
function createTreeview(selectedCat){    
    selectedCat = selectedCat || new Array();
    
    if(selectedCat){
        $.each(selectedCat, function(i, id_category){
            $('<input/>').attr({
                type: 'hidden',
                name: 'categoryBox[]',
                value: id_category.toString()
            }).appendTo($('#div_filterproductspro'));
        });
    }
        
    $("#categories-treeview").empty().removeClass().addClass('filetree').treeview({
